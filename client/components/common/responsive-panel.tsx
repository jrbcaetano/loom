...
export function ResponsivePanel({ isOpen, title, onClose, children, footer }: ResponsivePanelProps) {
  const { t } = useI18n();
  useEffect(() => {
    if (!isOpen) {
      return;
  ...
  return (
    <div className="loom-panel-overlay" role="presentation">
      ...
      <aside className="loom-panel-shell" role="dialog" aria-modal="true" aria-label={title}>
